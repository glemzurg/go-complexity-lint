...
func HighFanOut() { // want `function HighFanOut has fan out of 7 \(warn: >=7, fail: >=10\) \[warning\] \(reduce by extracting groups of related calls into helper functions; flat routing switches and one-call-per-field constructors are acceptable — consider //complexity:fanout:warn=N,fail=N override.\)`
	_ = dep.A()
	_ = dep.B()
	_ = dep.C()
	_ = dep.D()
...
func GoFuncFanOut() { // want `function GoFuncFanOut has fan out of 7 \(warn: >=7, fail: >=10\) \[warning\] \(reduce by extracting groups of related calls into helper functions; flat routing switches and one-call-per-field constructors are acceptable — consider //complexity:fanout:warn=N,fail=N override.\)`
	go func() {
		_ = dep.A()
		_ = dep.B()
		_ = dep.C()
