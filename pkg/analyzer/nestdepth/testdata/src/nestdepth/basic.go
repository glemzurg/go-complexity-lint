...
func DepthFive() {
	if true {
		for i := 0; i < 10; i++ {
			switch i {
			case 1:
				if true { // want `function DepthFive has a nesting depth of 5 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
					_ = i
				}
			}
		}
	...
	if true {
		for i := 0; i < 10; i++ {
			...
			case 1:
				if true {
					for j := 0; j < 5; j++ {
						if true { // want `function DepthSeven has a nesting depth of 7 \(warn: >=5, fail: >=7\) \[error\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
							_ = j
						}
					}
				}
