	...
	if true {
		for i := 0; i < 10; i++ {
			...
			case 0:
				select {
				case <-make(chan int):
					_ = i // want `function \*MyStruct\.DeepMethod has a nesting depth of 6 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
				}
			}
		}
	}
	...
	if true {
		fn := func() {
			for i := 0; i < 10; i++ {
				switch i {
				case 1:
					if true { // want `function ClosureNesting has a nesting depth of 6 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
						_ = i
					}
				}
			}
		...
		if v > 0 {
			...
			case 1:
				for _, w := range []int{4, 5} {
					if w > 0 { // want `function RangeLoop has a nesting depth of 6 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
						_ = w
					}
				}
			}
	...
	go func() {
		for i := 0; i < 10; i++ {
			...
			case 1:
				select {
				default:
					if true { // want `function GoRoutineClosure has a nesting depth of 7 \(warn: >=5, fail: >=7\) \[error\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
						fmt.Println(i)
					}
				}
			}
...
outer:
	for i := 0; i < 10; i++ {
		for j := 0; j < 10; j++ {
			if i > 0 {
				if j > 0 {
					if i+j > 15 { // want `function LabeledLoop has a nesting depth of 5 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
						break outer
					}
				}
			}
...
func BareBlock() {
	{
		if true {
			for i := 0; i < 10; i++ {
				switch i {
				case 1:
					if true { // want `function BareBlock has a nesting depth of 5 \(warn: >=5, fail: >=7\) \[warning\] \(reduce by extracting nested blocks into functions or using early returns/guard clauses\)`
						_ = i
					}
				}
			}
