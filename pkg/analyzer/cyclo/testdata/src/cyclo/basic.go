...
func FuncLitComplexity() { // want `function FuncLitComplexity has cyclomatic complexity of 10 \(warn: >=10, fail: >=15\) \[warning\] \(reduce by extracting logic into smaller functions or replacing conditional chains with table/map lookups; if this is a simple routing switch, consider override //complexity:cyclo:warn=N,fail=N Simple routing switch.\)`
	fn := func(x int) {
		if x > 0 {
			if x > 5 {
				if x > 10 {
...
func GoFuncComplexity() { // want `function GoFuncComplexity has cyclomatic complexity of 10 \(warn: >=10, fail: >=15\) \[warning\] \(reduce by extracting logic into smaller functions or replacing conditional chains with table/map lookups; if this is a simple routing switch, consider override //complexity:cyclo:warn=N,fail=N Simple routing switch.\)`
	go func(x int) {
		if x > 1 {
			_ = 1
		}
